...
def plot_gradient_at_train_points(X_train, f, ax=None):
    if ax is None:
        fig, ax = plt.subplots()

    # Compute gradient at training points
    gradients = np.array([np.gradient(f(x[None, :])) for x in X_train])

    # Plot gradient vectors
    ax.quiver(X_train[:, 0], X_train[:, 1], gradients[:, 0], gradients[:, 1], color='r')

    ax.set_title("Gradient Vectors at Training Points")
