...
class RealDataOptimization:
    def __init__(self, app,maxiter, n_repeats,deactivate_rec_scalar=False):
        ...
        self.params = {
            ...
            'offset_scale': 0.0253434383380036,
            'offset_range': 30.92663911701745,
            'protection_width': 3.2715701918611297,
            'n_iterations': 0,
            'randomseed': 524,
...
def main_no_rec(app,maxiter,n_repeats,plotting=False):
    ...
    avg_optimal_fxs, avg_cum_regrets,var_fxs = cbo.for_range_of_iter(plotting)
    print(f"Found optima: fx={avg_optimal_fxs}")
    print(f"With {n_repeats} repeats for every of the {maxiter} iterations ")
    if plotting:
...
if __name__ == '__main__':
    ...
    maxiter = 20
    n_repeats = 1
    avg_optimal_fxs,avg_cum_regrets,var_fxs= main_no_rec(app,maxiter, n_repeats,plotting=True)
